...
const FaceRecognition = () => {
  ...
  // Set interval to capture image every 5 seconds
  useEffect(() => {
  ...
  useEffect(() => {
    const sendImageToServer = async () => {
      if (capturedImage) {
        try {
          ...
          formData.append('reference_image_url', userImage);

          // Send form data to server
          const response = await http.POST(`detect/faceRecognition/${Examid}`, formData, {
            headers: {
          ...
          // Handle response
          if (response.data.message === "Not Matching") {
            dispatch(openToast({
              msg: "Face is not matching",
              type: "error",
            }));
          }
    ...
    sendImageToServer();
  }, [capturedImage, Examid, userImage, dispatch]);
